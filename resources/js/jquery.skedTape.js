...
(function (factory) {
  ...
  var SkedTape = function (opts) {
    ...
    this.$el.on(
      "click",
      ".sked-tape__event",
      $.proxy(this.handleEventClick, this)
    );
    this.$el.on(
      "contextmenu",
      ".sked-tape__event",
      $.proxy(this.handleEventContextMenu, this)
    );
    this.$el.on(
      "click",
      ".sked-tape__timeline-wrap",
      $.proxy(this.handleTimelineClick, this)
    );
    this.$el.on(
      "contextmenu",
      ".sked-tape__timeline-wrap",
      $.proxy(this.handleTimelineContextMenu, this)
    );
    this.$el.on(
      "mousemove",
      ".sked-tape__timeline-wrap",
    ...
    this.$el.on(
      "wheel",
      ".sked-tape__time-frame",
      $.proxy(this.handleWheel, this)
    );
    this.$el.on(
      "click",
      ".sked-tape__intersection",
      $.proxy(this.handleIntersectionClick, this)
    );
    this.$el.on(
      "contextmenu",
      ".sked-tape__intersection",
      $.proxy(this.handleIntersectionContextMenu, this)
    );
  };
  ...
  SkedTape.prototype = {
    ...
    collide: function (event) {
      for (var i = 0; i < this.events.length; ++i) {
        if (
          event.location == this.events[i].location &&
    ...
    addEvent: function (entry, opts) {
      ...
      if (!isValidTimeRange(start, end)) {
        throw new Error(
          "Invalid time range: " + JSON.stringify([entry.start, entry.end])
        );
      }
    ...
    renderLocation: function (location) {
      var $text = $('<div class="sked-tape__location-text"/>').text(
        location.name
      );
    ...
    renderTimeWrap: function (oldScroll) {
      ...
      this.$frame = $(
        '<div class="sked-tape__time-frame" tabindex="0"/>'
      ).appendTo($wrap);
    ...
    renderDates: function () {
      ...
      queue.forEach(function (item) {
        ...
        $("<li/>")
          ...
          .toggleClass(
            "sked-tape__date--short",
            proportion * duration <= SHORT_DURATION
          )
          .appendTo($ul);
      });
      return $ul;
        ...
        $.proxy(function (i, location) {
          ...
          events.forEach(function (event) {
            ...
            if (belongs && visible) {
              ...
              if (
                gap >= this.minTimeGapShown &&
                gap <= this.maxTimeGapShown &&
                !intersects
              ) {
    ...
    renderIntersections: function () {
      ...
      // Render the new ones
      this.$timeline.find(".sked-tape__event-row").each(
        $.proxy(function (i, row) {
          ...
          $.each(
            ...
            $.proxy(function (i, intersection) {
              if (
                intersection.end > this.start &&
                intersection.start < this.end
              ) {
    ...
    renderGap: function (gap, start, end) {
      ...
      var $text = $('<span class="sked-tape__gap-text"/>').text(
    ...
    updateDummyEvent: function () {
      ...
      this.$dummyEvent[0].className =
        "sked-tape__dummy-event " + (event.className || "");
      this.$dummyEvent.css({
      ...
      if (this.showIntermission) {
        ...
        if (prevEvent) {
          ...
          if (
            ...
            leftText +=
              "<br>+" + this.format.duration(interval * MS_PER_MINUTE);
          }
        }
    ...
    computeEventWidth: function (event) {
      ...
      return (
        (durationHours / getDurationHours(this.start, this.end)) * 100 + "%"
      );
    },
    computeEventOffset: function (event) {
      var hoursBeforeEvent = getDurationHours(this.start, event.start);
      return (
        (hoursBeforeEvent / getDurationHours(this.start, this.end)) * 100 + "%"
      );
    },
    updateTimeIndicatorsPos: function () {
      ...
      Object.keys(this.timeIndicators).forEach(function (locationId) {
        ...
        var tzOffset =
          location.tzOffset === undefined ? this.tzOffset : location.tzOffset;
    ...
    getIntersections: function (location) {
      ...
      var occupied = function (intersection) {
        for (var i = 0; i < intersections.length; ++i) {
          if (
            intersection.start.getTime() == intersections[i].start.getTime() &&
    ...
    render: function (opts) {
      var oldScrollLeft =
        opts && opts.preserveScroll && this.$frame && this.$frame.scrollLeft();
      ...
      setTimeout(
        $.proxy(function () {
          ...
          var template =
            ...
            '"></div>' +
            "</div>";
          this.$el.find(".sked-tape__event").each(
            $.proxy(function (i, el) {
              var $entry = $(el);
    ...
    pick: function (e) {
      var scalar =
        (e.pageX - this.$timeline.offset().left) / this.$timeline.width();
      var time =
        this.start.getTime() +
        scalar * (this.end.getTime() - this.start.getTime());
      var locationId;
    ...
    handleIntersectionContextMenu: function (e) {
      ...
      var jqEvent = this.makeMouseEvent(
    ...
    moveDummyEvent: function (picked) {
      ...
      if (this.snapToMins) {
        ...
        var min =
          left - lower < this.snapToMins / 2 ? lower : lower + this.snapToMins;
  ...
  var TWBS_MAJOR = $.fn.popover
    ? parseInt($.fn.popover.Constructor.VERSION.charAt(0), 10)
    : 0;
  ...
  function eventFromEvent(e) {
    return !!$(e.target).closest(
      ".sked-tape__event, .sked-tape__intersection"
    ).length;
  }
  ...
  $.fn.skedTape = function (opts) {
    var cmd =
      opts && (typeof opts === "string" || opts instanceof String) ? opts : "";
    ...
    return this.each(function () {
      ...
      if (!obj || !cmd) {
        ...
        opts.start &&
          opts.end &&
          obj.setTimespan(opts.start, opts.end, { update: false });
        opts.locations && obj.setLocations(opts.locations, { update: false });
        opts.events &&
          obj.setEvents(opts.events, { update: false, allowCollisions: true });
